...
pub struct RoundTransition {
    ...
    /// ``true`` if this round's [`Round::finalize`] may return [`FinalizeOutcome::Result`].
    pub may_produce_result: bool,
}

impl RoundTransition {
    ...
    ///
    /// This includes: the children rounds (if some nodes already finalized this round),
    /// the parent rounds (if those nodes still haven't finalized while we already have),
    /// and the sibling rounds (if some nodes went on a different path).
    pub(crate) fn simultaneous_rounds(&self, followed_by_echo_round: bool) -> BTreeSet<RoundId> {
    ...
    pub fn regular(round_num: u8) -> Self {
        Self {
    ...
    pub fn result(round_num: u8) -> Self {
        Self {
            id: RoundId::new(round_num),
            parents: if round_num > 1 {
                [RoundId::new(round_num - 1)].into()
...
pub trait Round<Id: PartyId>: 'static + Debug + Send + Sync {
    ...
    ///
    /// See [`RoundTransition`] documentation for more details.
